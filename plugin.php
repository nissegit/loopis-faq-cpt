 ...
 * Mostly useful if changing variable names of custom post tags identifiers 'faq' and taxomonies 'faq_kategorier'
...
function loopis_faq_cleanup() {
    ...
    //  Fetch and delete all categories in faq_kategori taxonomy
    $categories = get_terms([
        'taxonomy'   => 'faq_kategorier',
        'hide_empty' => false,
    ]);
    if (!is_wp_error($categories)) {
        foreach ($categories as $cat) {
            wp_delete_term($cat->term_id, 'faq_kategorier');
        }
    }

    // Fetch and delete all tags in faq_tag taxonomy
    $tags = get_terms([
        'taxonomy'   => 'faq_tag',
        'hide_empty' => false,
    ]);
